...
def take_command():
    try:
        ...
        print(f"Google recognition error: {error}")
        return ""
        ...
        print(f"Unexpected listening error: {error}")
        return ""
# ---------------------------------------------------------------------
# F. AI FALLBACK
# ---------------------------------------------------------------------
...
def main():
    ...
    try:
        ...
        print(f"Microphone initialization failed: {error}")
        return
    ...
    while True:
        try:
            ...
            command = query.replace("jarvis", "", 1).strip()
            ...
            print("\nJarvis stopped by the user.")
            break

        except Exception as error:
            print(f"Main-loop error: {type(error).__name__}: {error}")
